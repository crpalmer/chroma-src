...
import {hexStringToFloat, hexStringToInt, floatToHexString, intToHexString} from "../common/hex";
import OSpliceCountInstruction from "../instructions/o-splice-count";
import ODriveInformationInstruction from "../instructions/o-drive-information";
...
export default class RepRapEngine extends RaftEngine {
    ...
    public toRaftInstruction(line: GCodeLine): RaftInstruction {
        ...
        if (line.command === "O1") return fromO1(line);
        if (line.command === "O9") return fromO9(line);
        if (line.command === "O21" || line.command === "O22"
            || line.command === "O23" || line.command === "O24") return fromO21O22O23O24(line);
        if (line.command === "O25") return fromO25(line);
        if (line.command === "O26") return fromO26(line);
        if (line.command === "O27") return fromO27(line);
        if (line.command === "O28") return fromO28(line);
    ...
    public toOutputInstruction(instruction: RaftInstruction): GCodeLine {
        ...
        if (instruction instanceof ODriveInformationInstruction) return fromODriveInformation(instruction);
...
function fromO1(line: GCodeLine): OStartMulticolorInstruction {
    return new OStartMulticolorInstruction({
        comment: line.comment
    });
}
function fromO9(line: GCodeLine): OSoftResetInstruction {
    return new OSoftResetInstruction({
        comment: line.comment
    });
}
function fromO21O22O23O24(line: GCodeLine): OSpliceDataInstruction {
    return new OSpliceDataInstruction({
        drive: parseInt(line.command.substr(1)) - 21,
        length: hexStringToFloat(line.d.toString()),
        comment: line.comment
    });
}
function fromO25(line: GCodeLine): ODriveInformationInstruction {
    return new ODriveInformationInstruction({
        value: "" + line.k,
        comment: line.comment
    });
}
function fromO26(line: GCodeLine): OSpliceCountInstruction {
    return new OSpliceCountInstruction({
        count: hexStringToInt(line.d.toString()),
        comment: line.comment
    });
}
function fromO27(line: GCodeLine): OPingDataInstruction {
    return new OPingDataInstruction({
        length: hexStringToFloat(line.d.toString()),
        comment: line.comment
    });
}
function fromO28(line: GCodeLine): OAlgorithmCountInstruction {
    return new OAlgorithmCountInstruction({
        count: hexStringToInt(line.d.toString()),
        comment: line.comment
    });
}
...
function fromOSpliceData(instruction: OSpliceDataInstruction): GCodeLine {
    return {
        command: "O" + (instruction.drive + 21),
        d: floatToHexString(instruction.length),
        comment: instruction.comment
    };
}
function fromODriveInformation(instruction: ODriveInformationInstruction): GCodeLine {
    return {
        command: "O25",
        k: instruction.value,
        comment: instruction.comment
