...
const o_drive_information_1 = require("../instructions/o-drive-information");
...
class RepRapEngine extends base_1.default {
    ...
    toRaftInstruction(line) {
        ...
        if (line.command === "O21" || line.command === "O22"
            || line.command === "O23" || line.command === "O24")
            return fromO21O22O23O24(line);
        if (line.command === "O25")
            return fromO25(line);
        if (line.command === "O26")
            return fromO26(line);
    ...
    toOutputInstruction(instruction) {
        ...
        if (instruction instanceof o_drive_information_1.default)
            return fromODriveInformation(instruction);
...
function fromO1(line) {
    return new o_start_multicolor_1.default({
        comment: line.comment
    });
}
function fromO9(line) {
    return new o_soft_reset_1.default({
        comment: line.comment
    });
}
function fromO21O22O23O24(line) {
    return new o_splice_data_1.default({
        drive: parseInt(line.command.substr(1)) - 21,
        length: hex_1.hexStringToFloat(line.d.toString()),
        comment: line.comment
    });
}
function fromO25(line) {
    return new o_drive_information_1.default({
        value: "" + line.k,
        comment: line.comment
    });
}
function fromO26(line) {
    return new o_splice_count_1.default({
        count: hex_1.hexStringToInt(line.d.toString()),
        comment: line.comment
    });
}
function fromO27(line) {
    return new o_ping_data_1.default({
        length: hex_1.hexStringToFloat(line.d.toString()),
        comment: line.comment
    });
}
function fromO28(line) {
    return new o_algorithm_count_1.default({
        count: hex_1.hexStringToInt(line.d.toString()),
        comment: line.comment
    });
}
...
function fromODriveInformation(instruction) {
    return {
        command: "O25",
        k: instruction.value,
        comment: instruction.comment
